...
speckit-autopilot generates code in the correct language and framework by reading `docs/tech-stack.md`.

`bootstrap` generates this file automatically from `docs/product.md` (which already contains a `## Tech Stack` section written by `generate`). You can also regenerate it at any time with `/generate-techstack` — the previous file is backed up before overwriting.

If `docs/tech-stack.md` is absent when running `ship` or `audit`, the process stops with a clear error message.
...
Review and edit the file after `bootstrap` if the inferred stack needs adjusting.
...
| `docs/tech-stack.md` | Generated by `bootstrap` (or `/generate-techstack`); review and edit before running `ship` |
